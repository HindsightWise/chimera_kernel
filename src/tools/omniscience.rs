...
pub async fn run_omniscient_sweep(categories: Vec<&str>, max_results: usize, fetch_new_only: bool) {
    ...
    for category in categories {
        let sort_by = if fetch_new_only { "submittedDate" } else { "relevance" };
        let url = format!(
...
async fn refine_and_ingest(category: &str, title: &str, summary: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = match CreateChatCompletionRequestArgs::default()
        ...
        .messages([
            ChatCompletionRequestSystemMessageArgs::default().content(instructions).build()?.into(),
            ChatCompletionRequestUserMessageArgs::default().content(user_msg).build()?.into(),
        ])
        .temperature(0.1)
        .build() {
            Ok(r) => r,
    ...
    if let Ok(response) = tokio::time::timeout(tokio::time::Duration::from_secs(120), local_client.chat().create(request)).await {
            ...
            if let Some(choice) = res.choices.first() {
                if let Some(content) = &choice.message.content {
                    ...
                    if let Some(mem_pipeline) = crate::architecture::GLOBAL_MEM_PIPELINE.get() {
                        ...
                        let _ = mp.store_working(content).await;
